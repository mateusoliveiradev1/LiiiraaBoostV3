...
fn ipc_pipe_policy_is_local_only_and_has_an_explicit_service_dacl() {
    ...
    let admission_sddl = PipeSecurityPolicy::service_admission_sddl("S-1-5-80-424242");
    assert!(admission_sddl.contains("(A;;GR;;;IU)"));
    assert!(!admission_sddl.contains("(A;;GW;;;IU)"));
    assert!(!storage_sddl.contains(";;;IU)"));
}
