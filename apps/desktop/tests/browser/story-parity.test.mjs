...
const deriveCoverage = (catalog, manifest, referenceCatalog = catalog) => {
  const diagnostics = [];
  const scenarios = Array.isArray(catalog.scenarios) ? catalog.scenarios : [];
  const referenceScenarios = new Map(
    referenceCatalog.scenarios.map((scenario) => [scenario.id, scenario]),
  );
  ...
  for (const scenario of scenarios) {
    ...
    const referenceScenario = referenceScenarios.get(scenarioId);
    const referenceRoutes = new Set(referenceScenario?.requiredRoutes ?? requiredRoutes);
    const referenceStates = new Map();
    for (const requirement of referenceScenario?.requiredStates ?? scenario.requiredStates ?? []) {
      const statesForRoute = referenceStates.get(requirement.route) ?? new Set();
      statesForRoute.add(requirement.state);
      referenceStates.set(requirement.route, statesForRoute);
    }
...
describe('story parity canonical derivation', () => {
  ...
  it('story parity reports stable renamed and duplicate route/state diagnostics', () => {
    ...
    expect(deriveCoverage(renamedRoute, manifest, catalog).diagnostics).toContain(
    ...
    expect(deriveCoverage(renamedState, manifest, catalog).diagnostics).toContain(
    ...
    expect(deriveCoverage(duplicatePair, manifest, catalog).diagnostics).toContain(
