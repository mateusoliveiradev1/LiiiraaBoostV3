...
use physical_runner::{
    parse_runner_args, ArtifactCustody, FriendsRosterBinding, PhysicalGuestRunner,
    PhysicalRunConfig, PhysicalRunnerError, PhysicalRunnerIo, PhysicalRunnerState, PhysicalStage,
    RecordKind, RunPaths, TauriCommandSet, TRUSTED_INSTALLER_SPKI_SHA256,
};
...
impl PhysicalRunnerIo for FakeIo {
    ...
    fn current_executable_sha256(&mut self) -> Result<String, PhysicalRunnerError> {
        ...
        Ok(format!("sha256:{}", "c".repeat(64)))
    }

    fn install_msi(&mut self, executable: &str, args: &[String]) -> Result<(), PhysicalRunnerError> {
...
fn custody_failure_blocks_before_install_or_process_launch() {
    ...
    assert_eq!(io.events, vec![format!("verify-artifact:{TRUSTED_INSTALLER_SPKI_SHA256}")]);
}
...
fn first_invocation_installs_with_fixed_array_and_stops_at_installed_ready() {
    ...
    assert!(io.events.contains(&"write-record:InstalledReady".to_owned()));
    assert!(!io.events.iter().any(|event| event.starts_with("webdriver:")));
}

#[test]
fn clean_vm_refuses_cycle_until_bridge_checkpoint_record_exists() {
    let mut io = FakeIo::default();
    io.records.insert(RecordKind::InstalledReady, b"installed".to_vec());
...
fn resume_verifies_identity_and_observes_before_restore_without_apply_redispatch() {
    let mut io = FakeIo::default();
    io.records.insert(RecordKind::InstalledReady, b"installed".to_vec());
    io.records.insert(RecordKind::CheckpointReady, b"checkpoint".to_vec());
    io.records.insert(RecordKind::Continuation, b"continuation".to_vec());
    let state = PhysicalGuestRunner::new(config(PhysicalStage::OwnerPc))
        .run(&mut io)
        .expect("resume should restore after observation");
    assert_eq!(state, PhysicalRunnerState::Completed);

    let verify_self = io.events.iter().position(|event| event == "verify-self").unwrap();
    let observation = io.events.iter().position(|event| event == "observe").unwrap();
    let restore = io
        .events
        .iter()
