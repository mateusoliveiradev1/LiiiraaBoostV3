...
|---|---|---|---|
| Local Authenticode integrity, clean installation, update behavior, and non-elevated process boundary | UX-01 | Requires the self-signed development artifact and clean Windows machines; its root is untrusted, it has no SmartScreen reputation, and it is not distributable | Install only on controlled Windows 10/11 test images; verify cryptographic signature presence and the expected self-signed/untrusted-root chain, confirm no SmartScreen or public-trust claim, confirm the artifact is marked not distributable, then verify normal-user launch, capability denial, upgrade, and uninstall residue |
