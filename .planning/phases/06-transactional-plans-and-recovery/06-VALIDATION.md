...
| 06-31 | PLAN-01,05–08 | T-06-31A/B/C | A real create-once MSI contains signed/hash-bound desktop, service, self-contained runner and exact drivers; live install/update/rollback/uninstall preserves recovery custody | Node mutation/dry-run suite plus mandatory Windows `build-and-smoke` bytes/lifecycle command |
| 06-32 | PLAN-01,05–08 | T-06-32A/B/C | Only the signed Rust runner envelope can become physical; TypeScript/callback/simulation/browser/fixture bytes and relabeling fail before mutation | physical-writer/evaluator Vitest adversarial suites plus closed `phase6:ingest-physical` command |
| 06-33 | PLAN-01–08 | T-06-33A/B/C | A self-contained Rust EXE verifies MSI/runner/drivers, drives installed Tauri through WebDriver and broker, and emits bounded raw evidence with no guest Node/toolchain | native `physical_runner` Cargo suite plus release Windows runner build |
| 06-34 | PLAN-01,05–08 | T-06-34A/B/C | Elevated Hyper-V orchestration is exact-target, artifact-bound, self-contained-runner only; unsafe target/runtime/command/credential/evidence fails before mutation | Hyper-V policy mutation suite and exact artifact/simulation non-mutating audit |
| 06-35 | PLAN-05–07 | T-06-35A/B/C | Generated installation manifest, CMS/SPKI trust, live hashes, monotonic version, canonical paths, and protected ACL fail closed before service listen | contract generation/corpus plus installed-manifest CMS/hash/ACL/downgrade mutation suite |
...
- [x] Physical reachability is explicit: 06-35 signed custody → 06-30 authenticated token-bearing host → 06-36 bounded user impersonation → 06-37 Tauri transport → 06-33 self-contained runner → 06-31 real MSI → 06-38 fresh simulation admission → 06-34 exact Hyper-V bridge.
- [x] Owner/friends stages invoke the same artifact-bound runner EXE locally and ingest only its raw envelope through `phase6:ingest-physical`; the deterministic TypeScript harness cannot be a physical source.

`wave_0_complete: true` means every missing test scaffold/gate has explicit pre-implementation ownership and an automated command in the 38-plan decomposition; it does not claim those future files have already been implemented.
