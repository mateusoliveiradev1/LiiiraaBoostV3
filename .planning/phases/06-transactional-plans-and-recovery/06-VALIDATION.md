...
| 06-28 | PLAN-01–08 | T-06-28A/B/C | Generated-valid pseudonymous roster plus same-SPKI detached CMS is frozen at config-fixed paths; runner matches one local member and binds roster hash into consent before export, with one later review per member | roster freeze/CMS verification + planned friends run/admitted gates + all-four-stage final evaluator |
...
| 06-31 | PLAN-01,05–08 | T-06-31A/B/C | Sole builder emits all three generated configs and a detached-CMS-signed artifact manifest; desktop/service/runner are installed while tauri-driver/msedgedriver remain mandatory portable-only roles; restricted service startup is WTS-free and only the authenticated connected pipe peer can own one bounded effect token/lease; live lifecycle preserves recovery custody | Node installed-vs-portable-role/config mutation suite, optimizer-service client-bound startup/token/zero-call/cleanup suites, 06-37 response-loss/reconnect proof, plus mandatory Windows `build-and-smoke` composition: administrator read denial for installed manifest, pinned canonical/CMS artifact bytes, direct identity of all three installed binaries, restricted service startup, and two real authenticated pipe probes |
| 06-32 | PLAN-01–08 | T-06-32A/B/C/D | Exact ordered PLAN-01..08 evaluator/schema/fixtures, explicit planned gates, CMS-first ingestion, same-SPKI create-once roster pair, and append-only per-machine collections reject omissions, aliases, relabeling, overwrite, and cardinality gaps | individual PLAN-02/03/04 omission tests plus physical-writer/evaluator CLI/roster/CMS adversarial Vitest suites |
| 06-33 | PLAN-01–08 | T-06-33A/B/C/D | Self-contained Rust EXE verifies artifact and roster CMS/SPKI/config/live bytes, locally associates exactly one friends participant, binds roster hash into pre-export consent, and closes same-command observation-first reboot continuation | native `physical_runner` roster/consent/lifecycle suite plus release Windows runner build |
...
| 06-35 | PLAN-05–07 | T-06-35A/B/C | Installed, portable, and friends-roster documents use detached CMS and one compiled SPKI; installed roles exclude drivers while portable roles require them; live hashes/AuthentiCode/ACL/version checks remain fail closed | installed/artifact/roster mutation suites plus Windows all-target compile |
...
| 06-38 | PLAN-01–08 | T-06-38A/B/C | A never-used artifact-bound version at least v3 completes full deterministic simulation and emits exactly ordered PLAN-01..08 before becoming the physical predecessor | simulation-writer exact-coverage/deterministic/evaluator tests plus live `phase6:simulate` and planned verifier |
| 06-39 | PLAN-01,05–08 | T-06-39A/B/C | TypeSpec generates closed installation/artifact/friends-roster/config/continuation documents; installed and portable role sets stay distinct and roster paths/membership/lifecycle are sealed across Schema/TypeScript/Rust parity | contract generation/drift plus TypeScript/Rust installed-role/portable-role/roster adversarial corpus |
...
- [x] 06-39 owns the generated InstallationManifest, ArtifactManifest, FriendsRoster, PhysicalRunConfig, and PhysicalContinuation contracts across JSON Schema, TypeScript validators/types, and Rust; installed roles are desktop/service/runner only, drivers are portable-only, and no downstream plan invents a DTO or config.
- [x] 06-35 owns detached-CMS/compiled-SPKI/live-byte verification for installation, portable artifact, and fixed-path friends roster plus explicit Cryptography/WinTrust Cargo features before service listen, staging, execution, or ingestion.
...
- [x] 06-31 physical v41 passed and published from commit `994994ec4e61b45013930a7f650aaf0b46918d68`: MSI `0.1.41`, artifact manifest `sha256:8789c54ca0a73e2f496fedb7710dae6eac4b1b4bad10864e0284b7591d607784`, real restricted-service broker lifecycle, rollback/downgrade `1603` proofs, preserved recovery custody, and residue-free uninstall are recorded in `06-31-SUMMARY.md` and the immutable artifact `lifecycle-report.json`.
- [x] Owner/friends stages invoke the same artifact-bound runner EXE and signed-manifest config before/after reboot. Friends additionally transport one config-fixed same-SPKI signed roster pair, match exactly one local participant without arguments, bind roster hash into local consent before export, ingest only that raw envelope through `phase6:ingest-physical`, and retain one run/consent/review per roster machine; deterministic TypeScript cannot be physical.
- [x] 06-32 owns the exact ordered `PHASE6_REQUIREMENTS = PLAN-01..08`, matching schema enum/min/max and fixtures, with individual PLAN-02/03/04 omission tests; 06-38 emits precisely those eight IDs.
